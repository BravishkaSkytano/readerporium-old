 ...
 * @param optional string lang content language. Default (empty) is empty (let Amazon decide); set e.g. 'en_GB' for English
 * @return array ads[0..n] of array of strings url,title,img,price
 * @todo Adjust cachedate on merge
 */
function getAds($criteria,$lang='') {
  ...
  if ( !empty($lang) ) $amazon->setLangPref($lang);
  if ( !empty($GLOBALS['ads_asap_imgbase']) ) $amazon->setImgBase($GLOBALS['ads_asap_imgbase']);
...
function getAdBlock($ads) {
  if ( !empty($ads['items']) ) {
    ...
    foreach($ads['items'] as $item) {
      ...
      if ( !isset($item['price']) || empty($item['price']) || !preg_match('![0-9\.\,]+!',$item['price']) ) $item['price'] = '';
