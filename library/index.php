...
/** Setup list of user-defined donation link info
 * @function get_donationinfo
 * @param string csvfile (full path and) name of the CSV file to read data from. Example file: doc/donationlinks.csv
 * @return array donationinfo associative array: name=>[link,img,alt,comment]
 */
function get_donationinfo($csvfile) {
   $csv = new csv(";",'"',TRUE,FALSE);
   $csv->import($csvfile);
   $list = [];
   foreach ($csv->data as $data) {
     $list[$data['name']] = [ 'link'=>$data['link_url'],'img'=>$data['image_url'],'alt'=>$data['alt_text'],'comment'=>$data['comment'] ];
   }
   return $list;
}
...
if ( empty($prefix) && empty($_REQUEST['action']) ) { // Startpage
    if ( $pageformat=='html' && $ads_asap_initial && !empty($ads_asap_pubkey) && !empty($ads_asap_privkey) && !empty($amazonID) ) { // care for ads
      ...
      $asap = getAds($ads_asap_default_string,$ads_asap_lang);
...
switch($prefix) {
    ...
    case 'tag_id':
        ...
        if ( $pageformat=='html' && $ads_asap_initial && !empty($ads_asap_pubkey) && !empty($ads_asap_privkey) && !empty($amazonID) ) { // care for ads
          ...
          $asap = @getAds($adstring,$ads_asap_lang); // need to hide error message for some genres
    ...
    case 'series_id':
        ...
        if ( $pageformat=='html' && $ads_asap_initial && !empty($ads_asap_pubkey) && !empty($ads_asap_privkey) && !empty($amazonID) ) { // care for ads
          ...
          $asap = @getAds($adstring,$ads_asap_lang); // need to hide error message for some terms
          $adblock = getAdBlock($asap);
        } else $adblock = '';
        if ( !empty($adblock) ) $t->set_var('ad_css','<LINK REL="stylesheet" TYPE="text/css" HREF="'.$relurl.'tpl/html/asap.css">');
        if ($pageformat=='html') $t->set_var('adblock',$adblock);
    ...
    case '':
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action']=='bookdetails') {
            ...
            if ($pageformat=='html') {
              $t->set_block('template','donationblock','donation');
              $t->set_block('donationblock','donaelemblock','donaelem');
            }
            set_basics($t);
            $t->set_var('back_to_authors',trans('back_to_authors'));
            ...
            if ( $pageformat=='html' ) {
              if ( !empty($GLOBALS['donationInfo']) && file_exists($GLOBALS['donationInfo'])) {
                $dona = get_donationInfo($GLOBALS['donationInfo']);
                $parsed = false; foreach ( $dona as $don ) {
                  $t->set_var('donation_url',$don['link']);
                  $t->set_var('donation_title',$don['alt']);
                  $t->set_var('donation_img',$don['img']);
                  $t->parse('donaelem','donaelemblock',$parsed);
                  $parsed = true;
                }
            ...
            if ( empty($book['comment']) ) {
                ...
                if ( $pageformat=='opds' ) {
                  $comm = preg_replace_callback('!(</?\w+)(.*?>)!', function ($word) { return strtolower($word[1]).$word[2]; }, $comm); // OPDS/XML wants tags lower-case!
                  $comm = preg_replace_callback('! (CLASS|ID|SRC|HREF|ALT)=!', function($word) { return ' ' . strtolower($word[1]); }, $comm); // Same for attributes
              ...
              switch ( strtolower($ads_bookdetails_type) ) {
                ...
                case 'asap':                                    // Amazon Simple Ads (ASAP)
                  if ( empty($ads_asap_pubkey) || empty($ads_asap_privkey) || empty($amazonID) ) $adblock = '';
                    ...
                    $asap = @getAds($adstring,$ads_asap_lang); // need to hide error message for some terms
                    if ( empty($asap['items']) ) $asap = @getAds($adstring1,$ads_asap_lang); // fallback; sometimes book-specific additions cause empty ads
                    $adblock = getAdBlock($asap);
                  }
                  if ( !empty($adblock) ) $t->set_var('ad_css','<LINK REL="stylesheet" TYPE="text/css" HREF="'.$relurl.'tpl/html/asap.css">');
                  break;
